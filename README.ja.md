...
以下のサンプルはそれぞれ異なる原画像を使い、Panel 118 が一枚ずつ独立した一回の生成で作成しました。AIメタデータは削除済みです。横長は左に実写、右にデザインを置く厳密な50:50、縦長は上に実写、下にデザインを置く厳密な50:50です。 サンプルの文案は各原画像に基づき英語で生成します。
...
主題と構造関係を理解 → 積極的に削減 → 再配置と切り取り → 石墨の輪郭・自然なハッチング・軽い灰調 → 余白と少量の文字で編集構図にする。

## 完成品の識別ポイント

- 白黒または自然な石墨灰調が主で、原画像由来1–2色を微量だけ任意に使います。
- 緩く少し素朴な輪郭に、自然なハッチングと少量のクロスハッチング。
- 被写体は偏心、端寄せ、浮遊、拡大、縮小、部分切り取りが可能。
- 余白は被写体と同じ重要性を持ち、埋めるより削ります。
- 少量の単語や短文は主題・場所・感情・比喩から作り、言語・字体・形式は固定しません。
- 写真複製、複雑な背景、過密な細部、機械的な配置、テンプレート感を避けます。
